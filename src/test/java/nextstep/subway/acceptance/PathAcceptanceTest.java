...
class PathAcceptanceTest extends BaseTestSetup {
    ...
    public void setUp() {
        ...
        일호선을_생성한다(시청역_id, 서울역_id, 10L, 10L);

        Long 이호선_id = 노선_추출기.단일_id_를_추출한다(이호선을_생성한다(시청역_id, 을지로입구역_id, 10L, 10L));
        구간을_추가한다(이호선_id, 을지로입구역_id, 을지로3가역_id, 10L, 10L);

        삼호선을_생성한다(을지로3가역_id, 충무로역_id, 10L, 10L);

        Long 사호선_id = 노선_추출기.단일_id_를_추출한다(사호선을_생성한다(서울역_id, 회현역_id, 10L, 10L));
        구간을_추가한다(사호선_id, 회현역_id, 명동역_id, 10L, 10L);
        구간을_추가한다(사호선_id, 명동역_id, 충무로역_id, 10L, 10L);
    }
